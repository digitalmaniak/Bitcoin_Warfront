...
export function createTracers(scene, battle) {
  ...
  function fire(side) {
    ...
    let src = null;
    for (let tries = 0; tries < 10 && !src; tries++) {
      const u = units[(Math.random() * units.length) | 0];
      if (u.alive && u.state === 1 && u.side === side) src = u;
    }
    if (!src) return false;
    const slot = slots[side].find((s) => !s.live);
    if (!slot) return false;
    const front = battle.front;
    const tx = 2 * front - src.x + (Math.random() - 0.5) * 4;
    const tz = src.z + (Math.random() - 0.5) * 8;
  ...
  function update(dt, aggression, onShot) {
    for (let side = 0; side < 2; side++) {
      const rate = Math.min(40, 2 + aggression[side] * 0.15);
      acc[side] += rate * dt;
      while (acc[side] >= 1) {
        acc[side] -= 1;
        if (fire(side) && onShot) onShot(side);
      }
      let n = 0;
